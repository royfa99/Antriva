...
async function run() {
  ...
  try {
    ...
    if (!identifier) {
      console.log("Penggunaan: node make-owner.js <nomor_whatsapp_atau_email>");
      console.log("Contoh: node make-owner.js 08123456789");
      return;
    }
    
    // Jika user menginput nomor WA, kita ubah jadi format email bawaan sistem
    ...
    if (users.length > 0) {
      await sql`UPDATE "user" SET role = 'owner' WHERE email = ${email}`;
      console.log(`Berhasil mengubah role akun (WhatsApp: ${identifier}) menjadi 'owner'.`);
    } else {
